 ...
 */
  ...
  if (!$selectedPlayer) { // Class report
    if (!$teamParticipation) { // Class report
      echo '<tr>';
      echo '<td></td>';
      echo '<td colspan="'. $categories->count.'">Suivi du travail</td>';
      echo '<td colspan="4">Planet Alert</td>';
      echo '</tr>';
      echo '<tr>';
      echo '<td></td>';
      ...
      echo '<tr>';
      echo '<td colspan="7"><h2>Team participation : '. $team .'</h2></td>';
      echo '</tr>';
      if ($limit) {
        echo '<tr>';
        echo '<td colspan="7"><h3>'.$input->urlSegment3.' last lessons.</h3></td>';
        echo '</tr>';
      }
      foreach($allPlayers as $player) {
        $vv = 0;
